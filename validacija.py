...
with tab1:
    ...
    df = fetch_data_base(url) # Klic funkcije za pridobivanje podatkov
    columns_to_extract = [df.columns[0]] + [col for col in df.columns if col.startswith('coreNetPosition')]
    df_net_pos = df[columns_to_extract]
    df_net_pos.columns = [col.replace('coreNetPosition_', '') for col in df_net_pos.columns]
    ...
    # ********************************** PLOT 3 *************************************
    st.write('### Neto pozicije Non-CORE držav:')
...
with tab2:
    ...
    ate_str = df['dateTimeUtc'].iloc[0].strftime('%d.%m.%Y')
    # ********************************** PLOT 1 *************************************
    ...
    sns.heatmap(heatmap_data, cmap="YlOrRd_r", annot=True, fmt=".1f", linewidths=2.5,
