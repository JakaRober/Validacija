...
with tab1:
    ...
    df_iberia = df_iberia.rename(index={"Portugal": "  Portugal".rjust(target_length)})
    df_nib = df_nib.rename(index={"Switzerland": "  Switzerland".rjust(target_length)})
    ...
    # SEVER
    fig_2 = plt.figure(figsize=(18, 3))
    sns.heatmap(df_sever, cmap='RdYlGn', annot=True, fmt='.0f',linecolor='black', linewidths=0.5, cbar = False, vmin=vmin,vmax=vmax)
    plt.gca().xaxis.set_ticks_position('top')
    plt.ylabel('Sever'); plt.xlabel(''); plt.yticks(rotation=0)
    st.pyplot(fig_2)

    fig_3 = plt.figure(figsize=(18, 2.5))
    sns.heatmap(df_vzhod, cmap='RdYlGn', annot=True, fmt='.0f',linecolor='black', linewidths=0.5, cbar = False, vmin=vmin,vmax=vmax)
    plt.ylabel('Vzhod'); plt.xlabel(''); plt.yticks(rotation=0)
    plt.gca().set_xticklabels([]); plt.gca().xaxis.set_ticks([])  # Remove x-axis ticks completely
    st.pyplot(fig_3)

    fig_4 = plt.figure(figsize=(15.5, 1))
    sns.heatmap(df_iberia, cmap='RdYlGn', annot=True, fmt='.0f',linecolor='black', linewidths=0.5, cbar = False, vmin=vmin,vmax=vmax, annot_kws={"size": 8})
    plt.ylabel('Iberia'); plt.xlabel(''); plt.yticks(rotation=0)
    plt.gca().set_xticklabels([]); plt.gca().xaxis.set_ticks([])  # Remove x-axis ticks completely
    st.pyplot(fig_4)

    fig_5 = plt.figure(figsize=(17, 2))
    sns.heatmap(df_nib, cmap='RdYlGn', annot=True, fmt='.0f',linecolor='black', linewidths=0.5, cbar = False, vmin=vmin,vmax=vmax, annot_kws={"size": 8})
    plt.ylabel('NIB'); plt.xlabel(''); plt.yticks(rotation=0)
    plt.gca().set_xticklabels([]); plt.gca().xaxis.set_ticks([])  # Remove x-axis ticks completely
    st.pyplot(fig_5)

    fig_6 = plt.figure(figsize=(18, 3))
    sns.heatmap(df_balkan, cmap='RdYlGn', annot=True, fmt='.0f',linecolor='black', linewidths=0.5, cbar = False, vmin=vmin,vmax=vmax)
    plt.ylabel('Balkan'); plt.xlabel(''); plt.yticks(rotation=0)
    plt.gca().set_xticklabels([]); plt.gca().xaxis.set_ticks([])  # Remove x-axis ticks completely
    st.pyplot(fig_6)
